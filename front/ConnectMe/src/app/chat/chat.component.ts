...
import { chatHeader } from '../Model/ChatHeader';
  ...
  ngOnInit() {
    ...
    this.subscriptionList.push(
      this.currrentUser$.subscribe(s=>{
        ...
        //let rspns = new Map<string, chatResponse[]>();
        let rspns = this.generalService.loadMessagesFromStore();
        for(var [name, value] of rspns.entries()){
        console.log(name);
          let msgsTemp:chatResponse[] = [];
          msgsTemp= this.sortMessages(value, "Time");
          this.chatList.push(new chatHeader(name,msgsTemp[0].text,msgsTemp[1].text));
