...
public interface ChatRepository extends CrudRepository<Chat, Integer> {
	...
	public Chat getChateadById(int chatid);
	...
	public List<Chat> getallChateadforauser(int userid);
	...
	public List<Chat> doesChatwithUseridexists (int mainuserid, int searchwithuserid);
	...
	public int[] getusersinchat (int chatid);
	
	@Transactional
	@Modifying
	@Query(value="update chat set chat.unread_message_no = ?1 where chat.id=?2", nativeQuery = true)
	public void UpdateunreadMessageNoOfAChat(int unreadMesssageNo, int chatid); 
	
	@Transactional
	@Modifying
	@Query(value="delete from user_chat where chatid=?1 and userid=?2", nativeQuery = true)
	public void deleteuserfromauserchatrelation(int chatid, int userid); 
}
